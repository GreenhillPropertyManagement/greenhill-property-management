...
// ---- Flatpickr lazy-init flags ----
let fp = null;                 // Flatpickr instance
let filterInitDone = false;    // Guard to init once
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    // Show filters: lazy init when wrapper becomes visible
    $(document).off('click.v4filters', '.v4-filter-engage').on('click.v4filters', '.v4-filter-engage', function (e) {
      e.preventDefault();
      $('.updated-ui').show();   // your show logic already
      ensureFilterInit();        // init / redraw calendar now that it's visible
    });
    ...
    $(document)
      ...
      .on("click", '[api="finance-v4"]', function () {
        ...
        // Ensure calendars are ready + set default preset (keeps visible + hidden in sync)
        ensureFilterInit();
...
/* ---- Lazy init helper for the filter panel ---- */
function ensureFilterInit() {
  // If the wrapper is hidden, give the browser a tick so styles apply
  setTimeout(() => {
    if (!filterInitDone) {
      if (typeof window.initFlatpickr === 'function') window.initFlatpickr();
      if (typeof window.initDefault === 'function') window.initDefault();
      filterInitDone = true;
    } else if (fp && typeof fp.redraw === 'function') {
      fp.redraw(); // Recompute layout when re-opening panel
    }
  }, 0);
}
...
function initLandlordFinances() {
  $('[api-form="finance-filter"]').on("submit", function(event) {
    ...
    $.ajax({
      ...
      data: formData,
      // contentType not needed for GET
      dataType: "json",
...
function generateArrearsReport() {
  ...
  $.ajax({
    ...
    error: function() {
      console.error('Error generating arrears report');
...
(function FinanceFilterPanel() {
  ...
  function getPresetRange(key) {
    ...
    switch (key) {
      ...
      case 'quarter_to_date':{ const qStartMonth = Math.floor(end.getMonth() / 3) * 3; start = new Date(end.getFullYear(), qStartMonth, 1); break; }
    ...
    return { start: toISO2(start), end: toISO2(end) };
  }
  ...
  window.initFlatpickr = function initFlatpickr() {
    ...
    // If user types manually, still mirror
  ...
  $presetList.on('click', '.preset-item', function () {
    ...
    if (key === 'custom') {
      // Let the user pick dates manually
      return;
    }
  ...
  window.initDefault = function initDefault() {
  ...
  // Public helper to programmatically switch preset (optional)
